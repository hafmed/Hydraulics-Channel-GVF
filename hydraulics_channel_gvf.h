#ifndef HYDRAULICS_CHANNEL_GVF_H
#define HYDRAULICS_CHANNEL_GVF_H
...
public:
    hydraulics_channel_gvf(QWidget *parent = nullptr);
...
public slots:
    ...
    /////-----HAF 19-8-2022-----
    double CriticalSlope(double yc);
    /////-----HAF 21-3-2023------------------
    double calculnequivalent(double y); //n équivalent
    /////
    void full_tableoutput(int numsteps, double xx[], double y2[], double zz[]);
    void plot_grapheoutput(int numsteps, double y2[]);
    void afficher_informationoutput(double yc, double yn, QString profiletype, double y2[]);
    void read_input();
