//le 3-4-2020 - 5-9-2020 - 25-3-2023
...
#include "ui_hydraulics_channel_gvf.h"
#include <QDebug>
    ...
    ui(new Ui::hydraulics_channel_gvf)
{
    ...
    //--------------------HAF 9-8-2021---------------------------
    QString fig_gvf_fr ="QLabel{ image: url(:/icons/GVF-Eqts-Fr.png);}";
    QString configPhysique_fr ="QLabel{ image: url(:/icons/ConfigPhysique_fr.png);}";
    /////QPixmap (":/icons/GVF-Eqts-Fr.png");
    QString fig_gvf_en = "QLabel{ image: url(:/icons/GVF-Eqts-En.png);}";
    QString configPhysique_en ="QLabel{ image: url(:/icons/ConfigPhysique_en.png);}";
    ...
    if (locale!="fr")
    {
        ...
        ui->label_fig_configPhysique->setStyleSheet(configPhysique_en);
    }else{
        ui->label_figGVF->setStyleSheet(fig_gvf_fr);
        ui->label_fig_configPhysique->setStyleSheet(configPhysique_fr);
    }
    //------------------------HAF 25-7-2020----------------------
    ...
    ui->doubleSpinBox_coefManning_1->setLocale(QLocale(QLocale::English, QLocale::UnitedStates));
    ui->doubleSpinBox_coefManning_1->setGroupSeparatorShown(false);
    ui->doubleSpinBox_coefManning_2->setLocale(QLocale(QLocale::English, QLocale::UnitedStates));
    ui->doubleSpinBox_coefManning_2->setGroupSeparatorShown(false);
    ui->doubleSpinBox_coefManning_3->setLocale(QLocale(QLocale::English, QLocale::UnitedStates));
    ui->doubleSpinBox_coefManning_3->setGroupSeparatorShown(false);
    ...
    connect(ui->doubleSpinBox_coefManning_1, SIGNAL(valueChanged(double)), this, SLOT(Affichage_ycyn_HAF()));
    connect(ui->doubleSpinBox_coefManning_2, SIGNAL(valueChanged(double)), this, SLOT(Affichage_ycyn_HAF()));
    connect(ui->doubleSpinBox_coefManning_3, SIGNAL(valueChanged(double)), this, SLOT(Affichage_ycyn_HAF()));
    ...
    /////in latex
    /////    \definecolor{F}{rgb}{0.71, 0.9, 0.98}
    hafcolor = QColor(179, 229, 250);; /////HAF 15-8-2022
...
void hydraulics_channel_gvf::read_input(){

    So=ui->doubleSpinBox_S0->value(); //Bed slope;
    Q=ui->doubleSpinBox_Q->value();
    y0=ui->doubleSpinBox_y0->value();
    B=ui->doubleSpinBox_b->value();
    SS=ui->doubleSpinBox_m->value();
    coefmanning=calculnequivalent(y0);
    z0=0;x0=0;
    stepdist=ui->doubleSpinBox_dx->value();
    totaldist=ui->doubleSpinBox_L->value();
    ...
    yn=normal_depth(So, coefmanning, Q, y0, Cm, B, SS);
    yc=critical_depth(Q, y0, g, B, SS);
    profiletype=get_profile(So,coefmanning,Q,g,Cm,B,SS,y0);

    numsteps=totaldist/fabs(stepdist);

    fesetround(FE_UPWARD);
    numsteps=nearbyint(numsteps);

    if(totaldist>numsteps*fabs(stepdist)){
        numsteps=numsteps+1;
        totaldist=numsteps*fabs(stepdist);
        numsteps=numsteps+1;
    }else{
        numsteps=nearbyint(numsteps)+1;
    }

    /////------HAF 15-4-2023--------Correction bug tableWidget------
    if((stepdist<0 && So<0) || (stepdist>0 && So>0)){
        z0=abs(totaldist*So);
    }
    /////-----------------------------------------------------------
...
void hydraulics_channel_gvf::afficher_informationoutput(double yc, double yn, QString profiletype, double y2[]){
    if (ui->comboBox_Methods->currentIndex()==0 || ui->comboBox_Methods->currentIndex()==1 || ui->comboBox_Methods->currentIndex()==3){
    ...
    if (ui->comboBox_Methods->currentIndex()==2){
    ////----------
...
void hydraulics_channel_gvf::plot_grapheoutput(int numsteps, double y2[]){
    ...
    ui->customPlot->graph(0)->setPen(QPen(hafcolor)); // line color blue for first graph
    ui->customPlot->graph(0)->setBrush(QBrush(hafcolor)); // first graph will be filled with translucent blue
    ui->customPlot->addGraph();
...
void hydraulics_channel_gvf::full_tableoutput(int numstepstable, double xx[], double y2[], double zz[]){

    resultatsfr0="i\tx[i]\ty2[i]\ty2+z[i]\n";

    ui->tableWidget->setColumnCount(3);
    ui->tableWidget->setRowCount(numstepstable+1);

    if (ui->comboBox_Methods->currentIndex()==0) headtext << "x" << "y NR" << "z+y";
    if (ui->comboBox_Methods->currentIndex()==1) headtext << "x" << "y RK4" << "z+y";
    if (ui->comboBox_Methods->currentIndex()==2) headtext << "x" << "y KM" << "z+y";
    if (ui->comboBox_Methods->currentIndex()==3) headtext << "x" << "y Trap" << "z+y";

    ui->tableWidget->setStyleSheet("QTableView {selection-background-color: green;}");
    ui->tableWidget->setHorizontalHeaderLabels(headtext);

    for(i = 0; i < numstepstable; i++)
    {
        ...
        resultatsfr0=resultatsfr0+"0\t"+QString::number(xx[i])+
                       "\t"+QString::number(y2[i])+
...
double hydraulics_channel_gvf::calculnequivalent(double y) //n équivalent
{
    double n1=ui->doubleSpinBox_coefManning_1->value();
    double n2=ui->doubleSpinBox_coefManning_2->value();
    double n3=ui->doubleSpinBox_coefManning_3->value();
    double B=ui->doubleSpinBox_b->value();
    double SS=ui->doubleSpinBox_m->value();
    double n = pow((y*sqrt(SS*SS+1)*pow(n1, 3.0/2.0)+B*pow(n2, 3.0/2.0)+y*sqrt(SS*SS+1)*pow(n3, 3.0/2.0))/(B+2*y*sqrt(SS*SS+1)),2.0/3.0);
    return(n);
}
double hydraulics_channel_gvf::CriticalSlope(double yc)
{
    double Q=ui->doubleSpinBox_Q->value();
    double B=ui->doubleSpinBox_b->value();
    double SS=ui->doubleSpinBox_m->value();
    double Ac = yc*B + SS*yc*yc;
    double Pc = B + 2*yc*sqrt(SS*SS + 1);
    double Rc = Ac/Pc;
    double Cm;
    if (ui->radioButton_SI->isChecked()){
        Cm=1;
    ...
    double coefmanning=calculnequivalent(yc);

    Sc=pow((coefmanning*Q)/(Cm*Ac*pow(Rc,2./3)),2);
    return Sc;
}
...
void hydraulics_channel_gvf::methode_Trap(){
    ...
    for (i=1; i<numsteps; i++)
    {
        ...
        j=0;
        coefmanning=calculnequivalent(y2[i-1]);
        ...
        if (isnan(y2[i]) || y2[i]<0 || (y2[i-1]<yc && y2[i]>yc) || (y2[i-1]>yc && y2[i]<yc)){ // la 3eme condition : pour eviter que la surface libre coupe yc (en S1, M3,...)
            //y2[i]=nan("");
            pd.close();
            QMessageBox::critical(this, tr("Erreur"),
            ...
            numstepstable=i;break;
        }
        numstepstable=numsteps;
    }

    full_tableoutput(numstepstable, xx, y2, zz);

    afficher_informationoutput(yc, yn, profiletype, y2);

    plot_grapheoutput(numsteps, y2);
...
void hydraulics_channel_gvf::methode_Bresse()
{
    ...
    coefmanning=calculnequivalent(y[0]);

    yn=normal_depth(So, coefmanning, Q, y[0], Cm, b, 0);
    yc=critical_depth(Q, y[0], g, b, 0);
    profiletype=get_profile(So,coefmanning,Q,g,Cm,b,SS,y[0]);
    ...
    for (i=1; i<=npt; i++)
    {
        ...
        coefmanning=calculnequivalent(y[i]);
...
{ 
    loop_step_haf(So,coefmanning,Q,Cm,g,y0,B,SS,z0,x0,stepdist);
...
void hydraulics_channel_gvf::methode_DirectStep(){
    ...
    yn=normal_depth(So, coefmanning, Q, y[0], Cm, B, SS);
    yc=critical_depth(Q, y[0], g, B, SS);
    profiletype=get_profile(So,coefmanning,Q,g,Cm,B,SS,y[0]);
    ...
    for (i=1; i<=npt; i++)
    {
        ...
        double coefmanning1=calculnequivalent(y[i-1]);

        double coefmanning2=calculnequivalent(y[i]);

        Sf1=pow((coefmanning1*Q)/(Cm*A1*pow(R1,2./3)),2);
        Sf2=pow((coefmanning2*Q)/(Cm*A2*pow(R2,2./3)),2);
        Sfbar=(Sf1+Sf2)/2;
        dx[i]=(E2-E1)/(So-Sfbar);
        ///
        x[i]=x[i-1]+dx[i];
...
void hydraulics_channel_gvf::remplire_Colonne_Yinpt(int npt){
    ...
    for (i=1; i<=npt; i++)//15-6-2020 ubuntu
    {
...
void hydraulics_channel_gvf::afficher_information(double yc,double yn,
    ...
    if (profiletype=="M1"||profiletype=="M2"||profiletype=="M3") {
        if (profiletype=="M1"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_en.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1_en.png);}");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_fr.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1_fr.png);}");
            }
        }
        if (profiletype=="M2"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_en.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M2_en.png);}");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_fr.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M2_fr.png);}");
            }
        }
        if (profiletype=="M3"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_en.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M3_en.png);}");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M1M2M3_fr.png);}");
                ui->label_exempleCourbeRemous->setStyleSheet("QLabel{ image: url(:/icons/M3_fr.png);}");
            }
        }
    }
    if (profiletype=="C1"||profiletype=="C3") {
        if (profiletype=="C1"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/C1C3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/C1C3_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/C1C3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/C1_fr.png);");
            }
        }
        if (profiletype=="C3"){
            if (locale!="fr")
            {
                ...
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/C1C3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/C3_fr.png);");
            }
        }
    }
    if (profiletype=="S1"||profiletype=="S2"||profiletype=="S3") {
        if (profiletype=="S1"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S1_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S1_fr.png);");
            }
        }
        if (profiletype=="S2"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S2_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S2_fr.png);");
            }
        }
        if (profiletype=="S3"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S3_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/S1S2S3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/S3_fr.png);");
            }
        }
    }
    if (profiletype=="H2"||profiletype=="H3") {
        if (profiletype=="H2"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/H1H2H3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/H2_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/H1H2H3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/H2_fr.png);");
            }
        }
        if (profiletype=="H3"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/H1H2H3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/H3_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/H1H2H3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/H3_fr.png);");
            }
        }
    }
    if (profiletype=="A2"||profiletype=="A3") {
        if (profiletype=="A2"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/A1A2A3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/A2_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/A1A2A3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/A2_fr.png);");
            }
        }
        if (profiletype=="A3"){
            if (locale!="fr")
            {
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/A1A2A3_en.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/A3_en.png);");
            }else{
                ui->label_typeCourbeRemous->setStyleSheet("image: url(:/icons/A1A2A3_fr.png);");
                ui->label_exempleCourbeRemous->setStyleSheet("image: url(:/icons/A3_fr.png);");
            }
        }
    }
...
double* hydraulics_channel_gvf::loop_step_haf(double So, double n, double Q, double Cm,
                                              double g, double y0, double B, double SS, double z0, double x0,
                                              double stepdist)
{
    y2[0]=y0;
    xx[0]=x0;
...
void hydraulics_channel_gvf::Affichage_ycyn_HAF(){
    ...
    double coefmanning=calculnequivalent(y0);
    ...
    yc=critical_depth(Q, y0, g, b, SS);
    Sc=CriticalSlope(yc);
    ui->label_Sc->setText("Sc="+QString::number(Sc));
    if(ui->doubleSpinBox_S0->value()<=0){
        ui->label_yn->hide();
        ui->label_unite_yn->hide();
    }else{
...
void hydraulics_channel_gvf::on_pushButton_about_clicked()
{
    ...
    QString pubabout=tr("Avec différentes méthodes numériques cette application simule l’écoulement graduellement varié (GVF) : Calcul des courbes de remous ; \n")+
            ...
            tr("Programmé avec C++ (MinGW) avec comme IDE (Qt Creator) et avec ")+
